...
import torchvision
from utils import save_synthesized_images_labelwise
...
def main():
    ...
    if args.synthesize_data:
        ...
        print('[BEGIN] Zero Shot Knowledge Distillation For Image Classification')
        args.synthetic_data_path.parent.mkdir(parents=True, exist_ok=True)
        file_count_labelwise = np.zeros(10, dtype=int)

        for synthetic_data in zskd.synthesize():
            x = synthetic_data[0].detach().cpu()
            y = synthetic_data[1].argmax(dim=1).detach().cpu().numpy()
    ...
    if args.train_student:
        ...
        print('[BEGIN] Train Student Model')
